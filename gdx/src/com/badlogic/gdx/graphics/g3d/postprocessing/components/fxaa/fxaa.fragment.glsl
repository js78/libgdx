...
uniform sampler2D u_texture; //redered scene texture
uniform float u_textureWidth; //texture width
uniform float u_textureHeight; //texture height
...
vec4 PostFX(sampler2D tex, vec2 uv)
{
    ...
    //c.rgb = 1.0 - texture2D(u_texture, posPos.xy).rgb;
    c.a = 1.0;
    return c;
}
